...
const RatPage = () => {
  ...
  const sizeFilter = size
    ? parseFloat(size) >= rat.size.min && parseFloat(size) <= rat.size.max
    : true;

  const primaryColorFilter =
    selectedPrimaryColors.length === 0 ||
    selectedPrimaryColors.some((color) => rat.primaryColor.includes(color));

  const secondaryColorFilter =
    selectedSecondaryColors.length === 0 ||
    selectedSecondaryColors.some((color) => rat.secondaryColor?.includes(color));

  const thirdColorFilter =
    selectedThirdColors.length === 0 ||
    selectedThirdColors.some((color) => rat.thirdColor?.includes(color));

  const displayedRat = sizeFilter && primaryColorFilter && secondaryColorFilter && thirdColorFilter;
    ...
    <Container fluid>
      <Row>
        ...
        <Col md={3} className="border-end">
          <h4>Rat Details, Price: {rat.price}
          </h4>
        ...
        <Col md={9}>
          <Row>
            <Col className="d-flex justify-content-between align-items-center mb-3">
              <Card.Img variant="top" src={rat.image} />
            </Col>
          </Row>
        </Col>
      </Row>
