...
const MechPage = () => {
  ...
  const sizeFilter = size
    ? parseFloat(size) >= mech.size.min && parseFloat(size) <= mech.size.max
    : true;

  const primaryColorFilter =
    selectedPrimaryColors.length === 0 ||
    selectedPrimaryColors.some((color) => mech.primaryColor.includes(color));

  const secondaryColorFilter =
    selectedSecondaryColors.length === 0 ||
    selectedSecondaryColors.some((color) => mech.secondaryColor?.includes(color));

  const thirdColorFilter =
    selectedThirdColors.length === 0 ||
    selectedThirdColors.some((color) => mech.thirdColor?.includes(color));

  const displayedMech = sizeFilter && primaryColorFilter && secondaryColorFilter && thirdColorFilter;
    ...
    <Container fluid>
      <Row>
        ...
        <Col md={3} className="border-end">
          <h4>Mech Dragon Details, Price: {mech.price}
          </h4>
        ...
        <Col md={9}>
          <Row>
            <Col className="d-flex justify-content-between align-items-center mb-3">
              <Card.Img variant="top" src={mech.image} />
            </Col>
          </Row>
        </Col>
      </Row>
