...
const ImperialPage = () => {
  ...
  const sizeFilter = size
    ? parseFloat(size) >= imperial.size.min && parseFloat(size) <= imperial.size.max
    : true;

  const primaryColorFilter =
    selectedPrimaryColors.length === 0 ||
    selectedPrimaryColors.some((color) => imperial.primaryColor.includes(color));

  const secondaryColorFilter =
    selectedSecondaryColors.length === 0 ||
    selectedSecondaryColors.some((color) => imperial.secondaryColor?.includes(color));

  const thirdColorFilter =
    selectedThirdColors.length === 0 ||
    selectedThirdColors.some((color) => imperial.thirdColor?.includes(color));

  const displayedImperial = sizeFilter && primaryColorFilter && secondaryColorFilter && thirdColorFilter;
    ...
    <Container fluid>
      <Row>
        ...
        <Col md={3} className="border-end">
          <h4>Imperial Dragon Details, Price: {imperial.price}
          </h4>
        ...
        <Col md={9}>
          <Row>
            <Col className="d-flex justify-content-between align-items-center mb-3">
              <Card.Img variant="top" src={imperial.image} />
            </Col>
          </Row>
        </Col>
      </Row>
