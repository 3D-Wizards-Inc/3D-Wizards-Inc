...
const FerretPage = () => {
  ...
  const sizeFilter = size
    ? parseFloat(size) >= frog.size.min && parseFloat(size) <= frog.size.max
    : true;

  const primaryColorFilter =
    selectedPrimaryColors.length === 0 ||
    selectedPrimaryColors.some((color) => frog.primaryColor.includes(color));

  const secondaryColorFilter =
    selectedSecondaryColors.length === 0 ||
    selectedSecondaryColors.some((color) => frog.secondaryColor?.includes(color));

  const thirdColorFilter =
    selectedThirdColors.length === 0 ||
    selectedThirdColors.some((color) => frog.thirdColor?.includes(color));

  const displayedFrog = sizeFilter && primaryColorFilter && secondaryColorFilter && thirdColorFilter;
    ...
    <Container fluid>
      <Row>
        ...
        <Col md={3} className="border-end">
          <h4>Frog Details, Price: {frog.price}
          </h4>
        ...
        <Col md={9}>
          <Row>
            <Col className="d-flex justify-content-between align-items-center mb-3">
              <Card.Img variant="top" src={frog.image} />
            </Col>
          </Row>
        </Col>
      </Row>
