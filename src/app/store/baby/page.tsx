...
const BabyPage = () => {
  ...
  const sizeFilter = size
    ? parseFloat(size) >= baby.size.min && parseFloat(size) <= baby.size.max
    : true;

  const primaryColorFilter =
    selectedPrimaryColors.length === 0 ||
    selectedPrimaryColors.some((color) => baby.primaryColor.includes(color));

  const secondaryColorFilter =
    selectedSecondaryColors.length === 0 ||
    selectedSecondaryColors.some((color) => baby.secondaryColor?.includes(color));

  const thirdColorFilter =
    selectedThirdColors.length === 0 ||
    selectedThirdColors.some((color) => baby.thirdColor?.includes(color));

  const displayedBaby = sizeFilter && primaryColorFilter && secondaryColorFilter && thirdColorFilter;
    ...
    <Container fluid>
      <Row>
        ...
        <Col md={3} className="border-end">
          <h4>Baby Dragon Details, Price: {baby.price}
          </h4>
        ...
        <Col md={9}>
          <Row>
            <Col className="d-flex justify-content-between align-items-center mb-3">
              <Card.Img variant="top" src={baby.image} />
            </Col>
          </Row>
        </Col>
      </Row>
